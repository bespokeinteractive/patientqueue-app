...
<script type="text/javascript">
    function handlePatientRowSelection() {
        this.handle = function (row) {
            window.location = emr.pageLink("maternityapp", "main", { "patientId" : row.patientId, "queueId" : row.id })
        }
    }
    ...
    var getPatientsFromQueue = function(){
        ...
        jq.getJSON(emr.fragmentActionLink("patientqueueapp", "patientQueue", "getPatientsInMaternityDeliveryRoomQueue"),
                {
                    'maternityDeliveryRoomConceptId': ${maternityDeliveryRoomConceptId},
                })
